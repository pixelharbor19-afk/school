...
const supabase = createClient(
  process.env.WORKERS_SUPABASE_URL!,
  process.env.WORKERS_SUPABASE_SERVICE_ROLE_KEY!,
);
...
async function blacklistWorker(worker: string) {
  const reuse_at = await getNext8AMPH();

  const { error } = await supabase
    .from("worker_limits")
    .upsert(
      { worker, reuse_at },
      { onConflict: "worker", ignoreDuplicates: false },
    );

  if (error) {
    console.error(`[WORKER] Failed to blacklist ${worker}:`, error);
    return;
  }

  console.log(`[WORKER] ⛔ limited ${worker} until ${reuse_at}`);
}

async function getActiveWorkers(workers: string[]) {
  const { data, error } = await supabase
    .from("worker_limits")
    .select("worker")
    .gt("reuse_at", new Date().toISOString());

  if (error) {
    console.error("[WORKER] Failed to check limits:", error);
    return workers;
  }

  const limited = new Set((data ?? []).map((row) => row.worker));

  return workers.filter((worker) => !limited.has(worker));
}

export async function workerProxyHealth(proxies: string[]) {
  const activeWorkers = await getActiveWorkers(proxies);
  const shuffledProxies = shuffle(activeWorkers);
