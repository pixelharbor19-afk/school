...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (
      ...
      logRequest(req, "CENTAURUS", 400, "missing params");
    ...
    if (
      ...
      logRequest(req, "CENTAURUS", 401, "Invalid token");
    ...
    if (!dubs.length) {
      ...
      const searchRes = await fetch(
        `https://backup.vidstuck.xyz/backend/database/search-moviebox?${searchParams.toString()}`,
        {
          cache: "no-store",
        },
      );
      ...
      if (!searchRes.ok) {
        logRequest(req, "CENTAURUS", 502, "ICARUS search failed");
      ...
      if (!searchData?.success || !searchData?.dubs?.length) {
        logRequest(req, "CENTAURUS", 404, "Unavailable");
    ...
    if (!selectedDub?.subjectId || !selectedDub?.detailPath) {
      logRequest(req, "CENTAURUS", 404, "Dub source not found");
    ...
    if (!shuffledProxy) {
      logRequest(req, "CENTAURUS", 502, "No proxy available");
    ...
    if (cachedSources?.sources?.length) {
      ...
      logRequest(req, "CENTAURUS", 200, "Cache hit");
    ...
    const res = await fetch(
      `https://backup.vidstuck.xyz/backend/database/moviebox?${params.toString()}`,
      {
        cache: "no-store",
      },
    );
    ...
    if (!res.ok) {
      logRequest(req, "CENTAURUS", res.status, "Main request failed");
    ...
    logRequest(req, "CENTAURUS", 200, "OK");
